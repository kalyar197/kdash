# data/btc_price.py
"""
Bitcoin OHLCV data fetcher using CoinAPI with incremental fetching
...
def get_metadata():
    ...
    return {
        ...
        'strokeWidth': 2,
        'description': 'Bitcoin OHLCV data with incremental fetching',
...
def fetch_from_coinapi(start_date, end_date):
    """
    Fetch BTC OHLCV data from CoinAPI for a specific date range.
    ...
    """
    print(f"[BTC Price] Fetching BTC OHLCV from CoinAPI: {start_date.date()} to {end_date.date()}")
    ...
    try:
        ...
        for candle in data:
            ...
            if (timestamp_str and
                ...
                print(f"[BTC Price] Warning: Incomplete OHLCV data for {timestamp_str}, skipping...")
        ...
        print(f"[BTC Price] Successfully fetched {len(raw_data)} OHLCV data points from CoinAPI")
        if raw_data:
            print(f"[BTC Price] Sample data: timestamp={raw_data[0][0]}, O=${raw_data[0][1]:.2f}, H=${raw_data[0][2]:.2f}, L=${raw_data[0][3]:.2f}, C=${raw_data[0][4]:.2f}, V={raw_data[0][5]:.2f}")

        return raw_data

    except requests.exceptions.RequestException as e:
        print(f"[BTC Price] API request failed: {e}")
        raise
    except Exception as e:
        print(f"[BTC Price] Error fetching from CoinAPI: {e}")
        raise

def get_data(days='365'):
    """
    Fetches Bitcoin FULL OHLCV data using incremental fetching strategy.
    ...
    5. Save merged result to historical_data/btc_price.json
    ...
    try:
        ...
        if needs_older:
            ...
            if oldest_timestamp:
                ...
                print(f"[BTC Price] Fetching OLDER data from {older_start_date.date()} to {oldest_date.date()}")
                ...
                print(f"[BTC Price] Fetching ALL data from {older_start_date.date()} to {end_date.date()}")
        ...
        if historical_data:
            ...
            end_date = datetime.now(tz=timezone.utc)
            print(f"[BTC Price] Fetching NEWER data from {start_date.date()} to {end_date.date()}")
        ...
        if not is_valid:
            print(f"[BTC Price] Warning: Data structure validation failed: {error_msg}")
        ...
        if filtered_data:
            print(f"[BTC Price] Returning {len(filtered_data)} OHLCV records with 6 components each")
            print(f"[BTC Price] Date range: {datetime.fromtimestamp(filtered_data[0][0]/1000, tz=timezone.utc).date()} to {datetime.fromtimestamp(filtered_data[-1][0]/1000, tz=timezone.utc).date()}")
        else:
            print(f"[BTC Price] Warning: No data to return after filtering")
        ...
        print(f"[BTC Price] Error in get_data: {e}")

        # Fallback to historical data if available
        historical_data = load_historical_data(dataset_name)
        if historical_data:
            print(f"[BTC Price] Falling back to historical data ({len(historical_data)} records)")
        ...
        print(f"[BTC Price] No historical data available for fallback")
        return {
            'metadata': metadata,
            'data': [],
            'structure': 'OHLCV'
